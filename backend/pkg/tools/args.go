...
type TerminalAction struct {
	Input   string            `json:"input" jsonschema:"required" jsonschema_description:"Command to run in the container terminal. IMPORTANT: use single-quotes around values that contain spaces or special characters (e.g., FAKETIME='2026-04-26 10:00:00'). Never wrap such values in double-quotes as that causes JSON encoding failures. Prefer the 'env' field for environment variables instead of inline 'export' statements"`
	Env     map[string]string `json:"env,omitempty" jsonschema_description:"Optional environment variables to inject for this command (e.g., {\"LD_PRELOAD\": \"/usr/lib/x86_64-linux-gnu/faketime/libfaketime.so.1\", \"FAKETIME\": \"2026-04-26 10:00:00\"}). Use this instead of 'export KEY=VALUE' in 'input' to avoid shell quoting issues with values that contain spaces or special characters"`
	Cwd     string            `json:"cwd" jsonschema:"required" jsonschema_description:"Working directory for the command. Use empty string to use the default container directory (/work). Can be any absolute path; the directory will be created automatically if it does not exist. Do NOT use paths like /workspace — the container working root is /work"`
