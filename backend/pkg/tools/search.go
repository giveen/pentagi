...
func (s *search) Handle(ctx context.Context, name string, args json.RawMessage) (string, error) {
	...
	case SearchAnswerToolName:
		...
		filters := map[string]any{
			"flow_id":     strconv.FormatInt(s.flowID, 10),
		...
		docs := vectorSearch(ctx, logger, s.store, action.Questions, searchVectorStoreResultLimit, searchVectorStoreThreshold, filters)
