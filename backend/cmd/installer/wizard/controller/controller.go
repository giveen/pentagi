...
// llmProviderSpec captures all per-provider data in one place so that
// GetLLMProviderConfig / UpdateLLMProviderConfig / ResetLLMProviderConfig
// share a single source of truth and contain no switch statements.
type llmProviderSpec struct {
	name      string
	load      func(c *controller, cfg *LLMProviderConfig, embeddedPaths []string)
	save      func(c *controller, cfg *LLMProviderConfig) error
	resetVars []string
}

// llmProviderOrder defines the canonical display/iteration order.
var llmProviderOrder = []string{
	"openai", "anthropic", "gemini", "bedrock", "ollama",
	"deepseek", "glm", "kimi", "qwen", "custom",
}

// llmProviderRegistry maps provider ID → spec.
var llmProviderRegistry = map[string]llmProviderSpec{
	"openai": {
		name: "OpenAI",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("OPEN_AI_KEY")
			cfg.BaseURL, _ = c.GetVar("OPEN_AI_SERVER_URL")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL)
		},
		resetVars: []string{"OPEN_AI_KEY", "OPEN_AI_SERVER_URL"},
	},
	"anthropic": {
		name: "Anthropic",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("ANTHROPIC_API_KEY")
			cfg.BaseURL, _ = c.GetVar("ANTHROPIC_SERVER_URL")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL)
		},
		resetVars: []string{"ANTHROPIC_API_KEY", "ANTHROPIC_SERVER_URL"},
	},
	"gemini": {
		name: "Google Gemini",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("GEMINI_API_KEY")
			cfg.BaseURL, _ = c.GetVar("GEMINI_SERVER_URL")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL)
		},
		resetVars: []string{"GEMINI_API_KEY", "GEMINI_SERVER_URL"},
	},
	"bedrock": {
		name: "AWS Bedrock",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.Region, _ = c.GetVar("BEDROCK_REGION")
			cfg.DefaultAuth, _ = c.GetVar("BEDROCK_DEFAULT_AUTH")
			cfg.BearerToken, _ = c.GetVar("BEDROCK_BEARER_TOKEN")
			cfg.AccessKey, _ = c.GetVar("BEDROCK_ACCESS_KEY_ID")
			cfg.SecretKey, _ = c.GetVar("BEDROCK_SECRET_ACCESS_KEY")
			cfg.SessionToken, _ = c.GetVar("BEDROCK_SESSION_TOKEN")
			cfg.BaseURL, _ = c.GetVar("BEDROCK_SERVER_URL")
			cfg.Configured = cfg.DefaultAuth.Value == "true" ||
				cfg.BearerToken.Value != "" ||
				(cfg.AccessKey.Value != "" && cfg.SecretKey.Value != "")
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c,
				cfg.Region, cfg.DefaultAuth, cfg.BearerToken,
				cfg.AccessKey, cfg.SecretKey, cfg.SessionToken, cfg.BaseURL,
			)
		},
		resetVars: []string{
	...
	"ollama": {
		name: "Ollama",
		load: func(c *controller, cfg *LLMProviderConfig, embeddedPaths []string) {
			cfg.BaseURL, _ = c.GetVar("OLLAMA_SERVER_URL")
			cfg.APIKey, _ = c.GetVar("OLLAMA_SERVER_API_KEY")
			cfg.ConfigPath, _ = c.GetVar("OLLAMA_SERVER_CONFIG_PATH")
			cfg.HostConfigPath, _ = c.GetVar("PENTAGI_OLLAMA_SERVER_CONFIG_PATH")
			if slices.Contains(embeddedPaths, cfg.ConfigPath.Value) {
				cfg.HostConfigPath.Value = cfg.ConfigPath.Value
			}
			cfg.Model, _ = c.GetVar("OLLAMA_SERVER_MODEL")
			cfg.PullTimeout, _ = c.GetVar("OLLAMA_SERVER_PULL_MODELS_TIMEOUT")
			cfg.PullEnabled, _ = c.GetVar("OLLAMA_SERVER_PULL_MODELS_ENABLED")
			cfg.LoadModelsEnabled, _ = c.GetVar("OLLAMA_SERVER_LOAD_MODELS_ENABLED")
			cfg.Configured = cfg.BaseURL.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			if err := setVars(c,
				cfg.BaseURL, cfg.APIKey, cfg.Model,
				cfg.PullTimeout, cfg.PullEnabled, cfg.LoadModelsEnabled,
			); err != nil {
				return err
			}
			containerPath, hostPath := resolveConfigPath(
				cfg.HostConfigPath.Value, cfg.EmbeddedLLMConfigsPath, DefaultOllamaConfigsPath,
			)
			return setVars(c,
				loader.EnvVar{Name: cfg.ConfigPath.Name, Value: containerPath},
				loader.EnvVar{Name: cfg.HostConfigPath.Name, Value: hostPath},
			)
		},
		resetVars: []string{
	...
	"deepseek": {
		name: "DeepSeek",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("DEEPSEEK_API_KEY")
			cfg.BaseURL, _ = c.GetVar("DEEPSEEK_SERVER_URL")
			cfg.ProviderName, _ = c.GetVar("DEEPSEEK_PROVIDER")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL, cfg.ProviderName)
		},
		resetVars: []string{"DEEPSEEK_API_KEY", "DEEPSEEK_SERVER_URL", "DEEPSEEK_PROVIDER"},
	},
	"glm": {
		name: "GLM",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("GLM_API_KEY")
			cfg.BaseURL, _ = c.GetVar("GLM_SERVER_URL")
			cfg.ProviderName, _ = c.GetVar("GLM_PROVIDER")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL, cfg.ProviderName)
		},
		resetVars: []string{"GLM_API_KEY", "GLM_SERVER_URL", "GLM_PROVIDER"},
	},
	"kimi": {
		name: "Kimi",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("KIMI_API_KEY")
			cfg.BaseURL, _ = c.GetVar("KIMI_SERVER_URL")
			cfg.ProviderName, _ = c.GetVar("KIMI_PROVIDER")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL, cfg.ProviderName)
		},
		resetVars: []string{"KIMI_API_KEY", "KIMI_SERVER_URL", "KIMI_PROVIDER"},
	},
	"qwen": {
		name: "Qwen",
		load: func(c *controller, cfg *LLMProviderConfig, _ []string) {
			cfg.APIKey, _ = c.GetVar("QWEN_API_KEY")
			cfg.BaseURL, _ = c.GetVar("QWEN_SERVER_URL")
			cfg.ProviderName, _ = c.GetVar("QWEN_PROVIDER")
			cfg.Configured = cfg.APIKey.Value != ""
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			return setVars(c, cfg.APIKey, cfg.BaseURL, cfg.ProviderName)
		},
		resetVars: []string{"QWEN_API_KEY", "QWEN_SERVER_URL", "QWEN_PROVIDER"},
	},
	"custom": {
		name: "Custom",
		load: func(c *controller, cfg *LLMProviderConfig, embeddedPaths []string) {
			cfg.BaseURL, _ = c.GetVar("LLM_SERVER_URL")
			cfg.APIKey, _ = c.GetVar("LLM_SERVER_KEY")
			cfg.Model, _ = c.GetVar("LLM_SERVER_MODEL")
			cfg.ConfigPath, _ = c.GetVar("LLM_SERVER_CONFIG_PATH")
			cfg.HostConfigPath, _ = c.GetVar("PENTAGI_LLM_SERVER_CONFIG_PATH")
			if slices.Contains(embeddedPaths, cfg.ConfigPath.Value) {
				cfg.HostConfigPath.Value = cfg.ConfigPath.Value
			}
			cfg.LegacyReasoning, _ = c.GetVar("LLM_SERVER_LEGACY_REASONING")
			cfg.PreserveReasoning, _ = c.GetVar("LLM_SERVER_PRESERVE_REASONING")
			cfg.ProviderName, _ = c.GetVar("LLM_SERVER_PROVIDER")
			cfg.Configured = cfg.BaseURL.Value != "" && cfg.APIKey.Value != "" &&
				(cfg.Model.Value != "" || cfg.ConfigPath.Value != "")
		},
		save: func(c *controller, cfg *LLMProviderConfig) error {
			if err := setVars(c,
				cfg.BaseURL, cfg.APIKey, cfg.Model,
				cfg.LegacyReasoning, cfg.PreserveReasoning, cfg.ProviderName,
			); err != nil {
				return err
			}
			containerPath, hostPath := resolveConfigPath(
				cfg.HostConfigPath.Value, cfg.EmbeddedLLMConfigsPath, DefaultCustomConfigsPath,
			)
			return setVars(c,
				loader.EnvVar{Name: cfg.ConfigPath.Name, Value: containerPath},
				loader.EnvVar{Name: cfg.HostConfigPath.Name, Value: hostPath},
...
// setVars writes each EnvVar back through the state layer, returning the first error.
func setVars(c *controller, vars ...loader.EnvVar) error {
	for _, v := range vars {
		if err := c.SetVar(v.Name, v.Value); err != nil {
...
// resolveConfigPath maps a host-side path to the pair (containerPath, hostPath) that
// should be stored in the env file.
func resolveConfigPath(hostPath string, embeddedPaths []string, defaultContainerPath string) (string, string) {
	if hostPath == "" {
		return "", ""
	}
	if slices.Contains(embeddedPaths, hostPath) {
		return hostPath, ""
	}
	return defaultContainerPath, hostPath
}

// GetLLMProviders returns configured LLM providers
func (c *controller) GetLLMProviders() map[string]*LLMProviderConfig {
	result := make(map[string]*LLMProviderConfig, len(llmProviderOrder))
	for _, id := range llmProviderOrder {
		result[id] = c.GetLLMProviderConfig(id)
	}
	return result
}
...
func (c *controller) GetLLMProviderConfig(providerID string) *LLMProviderConfig {
	embeddedPaths := GetEmbeddedLLMConfigsPath(c.files)
	cfg := &LLMProviderConfig{
		Name:                   "Unknown",
		EmbeddedLLMConfigsPath: embeddedPaths,
	}
	if spec, ok := llmProviderRegistry[providerID]; ok {
		cfg.Name = spec.name
		spec.load(c, cfg, embeddedPaths)
	}
	return cfg
}
...
func (c *controller) ResetLLMProviderConfig(providerID string) map[string]*LLMProviderConfig {
	if spec, ok := llmProviderRegistry[providerID]; ok {
		if err := c.ResetVars(spec.resetVars); err != nil {
			return nil
		}
	}
